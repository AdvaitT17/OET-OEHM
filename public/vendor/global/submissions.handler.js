...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("currentSemesterGrid")
    .addEventListener("click", handleSubmissionClick);
  document
    .getElementById("previousSemestersGrid")
    .addEventListener("click", handleSubmissionClick);
  document.getElementById("closeModal").addEventListener("click", closeModal);
  document
    .getElementById("submissionForm")
    .addEventListener("submit", handleSubmissionSubmit);
...
function populateSubmissionsGrids(submissions) {
  const currentSemesterSubmissions = submissions.filter(
    (s) => s.enrolled_semester === userCurrentSemester
  );
  const previousSemesterSubmissions = submissions.filter(
    (s) => s.enrolled_semester < userCurrentSemester
  );

  if (currentSemesterSubmissions.length === 0) {
    document.getElementById("currentSemesterGrid").innerHTML =
      '<p class="no-submissions">No courses opted for this semester yet.</p>';
  } else {
    populateGrid("currentSemesterGrid", currentSemesterSubmissions);
  }

  if (previousSemesterSubmissions.length === 0) {
    document.getElementById("previousSemestersGrid").innerHTML =
      '<p class="no-submissions">No submissions from previous semesters.</p>';
  } else {
...
function createSubmissionCard(submission) {
  ...
  card.innerHTML = `
        <div class="card-header ${submission.type.toLowerCase()}">
            ...
            <span class="course-type course-type-${submission.type}">${
    submission.type
  }</span>
        ...
        <div class="card-body">
            ...
            <div class="card-info">
                ...
                <span class="submission-status ${submission.submission_status
                  .toLowerCase()
                  .replace(" ", "-")}">
    ...
    submission.submission_status
            ...
            <div class="card-info">
                ...
                <span>${
                  ...
                  submission.submission_link !== "Not submitted"
            ...
            ${
              submission.submission_status === "Not submitted" ||
              submission.submission_status === "Rejected"
                ? `<button class="btn btn-primary" data-course-id="${
...
function showSubmissionModal(courseId) {
  ...
  document.getElementById("submitBtn").textContent =
    submission.submission_link !== "Not submitted" ? "Update" : "Submit";
...
function isValidCourseraUrl(url) {
  const courseraRegex = /^https:\/\/coursera\.org\/share\/[a-zA-Z0-9]+$/;
...
function handleSubmissionSubmit(e) {
  ...
  if (!submissionLink) {
    showError("Please enter a submission link");
    return;
  }

  if (!isValidCourseraUrl(submissionLink)) {
    showError(
      "Please enter a valid Coursera share URL (https://coursera.org/share/...)"
    );
    return;
  }
